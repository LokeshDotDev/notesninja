...
export async function ProfessionalCategoryPage({ categoryName }: ProfessionalCategoryPageProps) {
  ...
  try {
    ...
    if (!dbCategory) {
      ...
      try { trackError("Category not found", "category_view"); } catch { } // ignore server track error
    } else {
      category = dbCategory as unknown as Category; // Safely cast matching fields
      try { trackCategoryView(dbCategory.name); } catch { }
    ...
    try { trackError("Failed to load category", "category_view"); } catch { }
  }
