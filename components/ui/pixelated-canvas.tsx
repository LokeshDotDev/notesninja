...
interface ExtendedImageElement extends HTMLImageElement {
  _cleanup?: () => void;
}
...
export const PixelatedCanvas: React.FC<PixelatedCanvasProps> = ({
  ...
  React.useEffect(() => {
    ...
    img.onload = () => {
      ...
      (img as ExtendedImageElement)._cleanup = cleanup;
    };
    ...
    if (responsive) {
      ...
      return () => {
        ...
        if ((img as ExtendedImageElement)._cleanup) (img as ExtendedImageElement)._cleanup?.();
      };
    }

    return () => {
      isCancelled = true;
      const imgElement = img as ExtendedImageElement;
      if (imgElement && imgElement._cleanup) {
        imgElement._cleanup();
      }
    };
  }, [
