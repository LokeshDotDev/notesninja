...
interface PurchaseDialogState {
  open: boolean;
  product: ApiPost | null;
}

interface DownloadDialogState {
  open: boolean;
  purchase: { id: string; userEmail: string; };
}
...
console.log("GalleryGridCombined received items:", items.length, items);
...
<div className='rounded-xl bg-gray-200 dark:bg-gray-700 w-full h-96 flex items-center justify-center'>
...
{downloadDialog.open && downloadDialog.purchase && (
<> <DownloadDialog
open={downloadDialog.open}
onOpenChange={(open) => setDownloadDialog(prev => ({ ...prev, open }))}
purchaseId={downloadDialog.purchase.id}
